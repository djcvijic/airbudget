...
suite("goals screen", function () {
    test("opens with the currency label and any existing goal prefilled", async function () {
        ...
        assertEqual(win.goalsCloseButton.style.display, "");
        assertEqual(win.goalsRevertButton.style.display, "none");
        assertEqual(win.goalsDoneButton.style.display, "none");
    });
    ...
    test("editing the amount swaps Done for Revert + Apply and hides the last-set line", async function () {
        var win = await freshApp(seededForGoals({ goalAmount: 500, goalSetDate: "2026-01-15" }));
        win.document.getElementById("open-goals-button").click();

        setValue(win.goalsAmountInput, "750");

        assertEqual(win.goalsCloseButton.style.display, "none");
        assertNotEqual(win.goalsRevertButton.style.display, "none");
    ...
    test("applying saves the goal, dates it today, and shows a confirmation toast", async function () {
        ...
        assertEqual(win.goalsCloseButton.style.display, "", "back to a clean Done state after applying");
    ...
    test("reverting discards the change", async function () {
        var win = await freshApp(seededForGoals({ goalAmount: 500, goalSetDate: "2026-01-15" }));
        win.document.getElementById("open-goals-button").click();
        setValue(win.goalsAmountInput, "999");

        win.goalsRevertButton.click();

        assertTrue(isActive(win.mainViewScreen));
        assertEqual(win.state.goalAmount, 500);
    });
    ...
    test("applying from the warning modal saves the edit and continues the navigation", async function () {
        var win = await freshApp(seededForGoals());
        win.document.getElementById("open-goals-button").click();
        setValue(win.goalsAmountInput, "1200");
        win.document.getElementById("open-home-button").click();

        win.document.getElementById("goals-unsaved-apply-button").click();
...
suite("goals screen: duration/date preview", function () {
    ...
    test("shows a message instead of a duration when budgets leave no expected savings", async function () {
        var win = await freshApp(seededForGoals({
            period: "weekly",
        ...
        assertEqual(win.goalsDurationOutputEl.textContent, "Your current budgets don't leave any expected savings.");
        assertEqual(win.goalsDateOutputEl.textContent, "");
    });

    test("an empty amount shows no preview", async function () {
        var win = await freshApp(seededForGoalsResults());
        win.document.getElementById("open-goals-button").click();
